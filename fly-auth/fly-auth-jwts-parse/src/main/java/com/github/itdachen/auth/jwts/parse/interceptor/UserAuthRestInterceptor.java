package com.github.itdachen.auth.jwts.parse.interceptor;
...
public class UserAuthRestInterceptor implements HandlerInterceptor {
    ...
    public UserAuthRestInterceptor(IVerifyTicketTokenHelper verifyTicketTokenService, JwtsProperties jwtsProperties) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        boolean passFlag = jwtsProperties.getVerifiedTicket().equals(header);
        if (!passFlag) {
            // 是否内部调用
            header = request.getHeader(UserInfoConstant.NAL_FEIGN);
            passFlag = jwtsProperties.getNalFeign().equals(header);
        }
        ...
        try {
            ...
            final IJwtsInfo ijwtInfo = verifyTicketTokenService.verifyTicketToken(token);
