<template>

  <lay-container fluid="true" class="fly-container">
    <lay-card class="fly-search-card">
      <lay-form>
        <lay-form-item label="层级代码" prop="levelCode" mode="inline">
          <lay-input v-model="queryDeptLevelParams.levelCode"></lay-input>
        </lay-form-item>
        <lay-form-item label="层级名称" prop="levelTitle" mode="inline">
          <lay-input v-model="queryDeptLevelParams.levelTitle"></lay-input>
        </lay-form-item>
        <lay-form-item label="有效标志" prop="validFlag" mode="inline">
          <lay-input v-model="queryDeptLevelParams.validFlag"></lay-input>
        </lay-form-item>
        <lay-form-item mode="inline">
          <lay-button class="fly-button fly-button-search" style="margin-left: 20px" type="primary" size="sm"
                      @click=" reloadDeptLevelDate(1, flyLayPage.limit) ">
            <lay-icon class="layui-icon-search"></lay-icon>
            查询
          </lay-button>
          <!--          <lay-button class="fly-button fly-button-reset" size="sm" @click=" resetQueryFormValue ">-->
          <!--            <lay-icon class="layui-icon-refresh"></lay-icon>-->
          <!--            重置-->
          <!--          </lay-button>-->
          <lay-button class="fly-button fly-button-exp" size="sm" @click="">
            <lay-icon class="layui-icon-export"></lay-icon>
            导出
          </lay-button>
        </lay-form-item>
      </lay-form>

    </lay-card>


    <!-- table   :default-toolbar="true"  :height="'100%'" line row nob -->
    <div class="table-box">
      <lay-table :page="false" :skin="'line'"
                 :height="'90%'"
                 :columns=" deptLevelColumns "
                 :loading=" tableDeptLevelData.loading "
                 :data-source=" tableDeptLevelData.rows "
                 @change=" reloadDeptLevelDate ">

        <template v-slot:toolbar>
          <lay-button size="sm" type="primary" class="fly-button fly-toolbar-addition" @click=" onTapDeptLevelAdd ">
            <lay-icon class="layui-icon-addition"></lay-icon>
            新增
          </lay-button>
        </template>

        <template v-slot:operator="{ row }">
          <lay-button size="sm" type="primary" class="fly-button fly-view-button" @click=" onTapDeptLevelView(row) ">
            <lay-icon class="layui-icon-list"></lay-icon>
            查看
          </lay-button>
          <lay-button size="sm" type="primary" class="fly-button fly-edit-button" @click=" onTapDeptLevelEdit(row) ">
            <lay-icon class="layui-icon-edit"></lay-icon>
            编辑
          </lay-button>

          <lay-button size="sm" type="danger" class="fly-button fly-remove-button"
                      @click=" removeDeptLevelHandler(row.id) ">
            <lay-icon class="layui-icon-delete"></lay-icon>
            删除
          </lay-button>

        </template>
      </lay-table>


      <lay-page v-model="flyLayPage.page"
                v-model:limit="flyLayPage.limit"
                :layout="flyLayPage.layout"
                :pages="flyLayPage.pages"
                :limits="flyLayPage.limits"
                theme="blue"
                :total=" tableDeptLevelData.total"
                @change="previousNextPage" style="margin-top: 15px"></lay-page>

    </div>
  </lay-container>


  <RefDeptLevel ref="refDeptLevelComponent"/>

</template>


<script setup lang="ts" name="deptLevelComponent">
import {ref, reactive, onMounted} from 'vue';
import RefDeptLevel from './RefDeptLevel.vue';

import useDeptLevelComposable from '@/composables/admin/DeptLevelComposable';

const {
  refDeptLevelComponent,
  deptLevel,
  flyLayPage,
  tableDeptLevelData,
  deptLevelColumns,
  queryDeptLevelParams,

  /* 加载表格 */
  reloadDeptLevelDate,
  loadTableDeptLevelData,

  /* 新增, 编辑, 查看按钮 */
  onTapDeptLevelAdd,
  onTapDeptLevelEdit,
  onTapDeptLevelView,

  /* 接口 */
  removeDeptLevelHandler,
  deptLevelDataHandler
} = useDeptLevelComposable();


/**
 * 上一页, 下一页
 * @param current
 * @param limit
 */
const previousNextPage = ({current = 1, limit = 10}) => {
  queryDeptLevelParams.page = current;
  queryDeptLevelParams.limit = limit;
  loadTableDeptLevelData(queryDeptLevelParams);
}


/**
 * 页面初始化, 加载数据
 */
onMounted(() => {
  reloadDeptLevelDate(1, flyLayPage.limit);
})


</script>


<style scoped>

</style>